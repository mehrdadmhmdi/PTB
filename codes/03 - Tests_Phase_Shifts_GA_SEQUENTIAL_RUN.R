#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Phase-shift quantile regression on gestational age t = GA (weeks)
# Model: Q_tau(Y | t, G) = alpha_tau + f_tau(t + phi_tau * G),
...
df <- data %>%
  mutate(
    GA = GA * 39,                 # t_{ij} in weeks (per your scaling)
...
fit_at_phi <- function(dat, yvar, tau, df_spline, phi,
                       Grouping_col = "G", t_col = "GA") {
  dat <- dat %>% mutate(x_shift = .data[[t_col]] + phi * .data[[Grouping_col]])
...
profile_phi <- function(dat, yvar, tau, df_spline = 3,
  ...
  for (k in seq_along(phi_grid)) {
    objs[k] <- fit_at_phi(dat, yvar, tau, df_spline, phi_grid[k], Grouping_col, t_col)$obj
  }
...
# Model: Q_tau(Y|t,G,ID)= alpha + f(t + (phi + b_ID)*G)
# ============================================================

rho_tau_sum <- function(u, tau) sum(u * (tau - (u < 0)))

fit_given_phi_b <- function(dat, yvar, tau, df_spline, phi, b_by_id,
                            id_col="Participant_ID", Grouping_col="G", t_col="GA") {
  bid <- as.character(dat[[id_col]])
  bvec <- ifelse(dat[[Grouping_col]] == 1L, b_by_id[bid], 0)
  bvec[is.na(bvec)] <- 0
  
  dat$x_shift <- dat[[t_col]] + (phi + bvec) * dat[[Grouping_col]]
  f <- as.formula(sprintf("%s ~ ns(x_shift, df=%d)", yvar, df_spline))
...
fit_phase_shift_RE_given_sigma <- function(dat, yvar, tau,
  ...
  G_by_id <- tapply(dat[[Grouping_col]], dat[[id_col]], function(z) z[1])
  ...
  for (it in seq_len(max_iter)) {
    ...
    for (k in seq_along(phi_grid)) {
      tmp <- fit_given_phi_b(dat, yvar, tau, df_spline, phi_grid[k], b_by_id,
                             id_col=id_col, Grouping_col=Grouping_col, t_col=t_col)
      objs_phi[k] <- tmp$obj
      mods_phi[[k]] <- tmp
    }
    ...
    # (2) update each PTB subject b_i
    for (sid in ptb_ids) {
      rows_i <- which(as.character(dat[[id_col]]) == sid & dat[[Grouping_col]] == 1L)
      ...
      for (kk in seq_along(b_grid)) {
        b_cand <- b_grid[kk]
        dat_i2 <- dat_i
        dat_i2$x_shift <- dat_i2[[t_col]] + (phi + b_cand)  # G=1 inside PTB
        yhat <- tryCatch(predict(mod_cur, newdata = dat_i2),
                         error = function(e) rep(NA_real_, length(y_i)))
        if (anyNA(yhat)) next
        u <- y_i - yhat
...
fit_phase_shift_RE <- function(dat, yvar, tau,
  ...
  for (sg in sigma_grid) {
    fit <- fit_phase_shift_RE_given_sigma(dat, yvar, tau,
                                          id_col=id_col, t_col=t_col, Grouping_col=Grouping_col,
                                          df_spline=df_spline, phi_grid=phi_grid, b_grid=b_grid,
                                          sigma=sg, max_iter=max_iter, tol=tol,
                                          seed=seed + round(100*sg))
...
# -----------------------------
# Cluster bootstrap CI for phi and sigma^2; test sigma^2=0 via wild cluster bootstrap under H0
# -----------------------------
phase_shift_cluster_boot <- function(df, yvar, tau,
  ...
  # H0 sigma^2 = 0 (fixed shift)
  obs0 <- profile_phi(dat, yvar, tau, df_spline, phi_grid, Grouping_col = Grouping_col, t_col = t_col)
  obj0 <- obs0$obj_min
  ...
  # wild cluster bootstrap test of sigma^2=0
  set.seed(seed + 999)
  dat0 <- dat %>% mutate(x_shift = .data[[t_col]] + obs0$phi_hat * .data[[Grouping_col]])
  ...
  for (bb in seq_len(B_sigma0)) {
    ...
    b0 <- profile_phi(dat_star, yvar, tau, df_spline, phi_grid, Grouping_col = Grouping_col, t_col = t_col)
...
summ <- run_phase_shift_grid(df, vars2, taus, ptb_level=1, B=100, B_sigma0=100)
write.csv(summ, "Test_results/qr_phase_shift_summary_GA.csv", row.names=FALSE)
